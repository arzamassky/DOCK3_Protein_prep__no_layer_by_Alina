...
def get_blaster_steps(blaster_files, flat_param_dict, working_dir):
    ...
    if flat_param_dict["thin_spheres_elec.use"]:
        ...
        # === NO DIELECTRIC LAYER PATH ===
        # Do NOT generate low-dielectric spheres. Instead, write an empty PDB to the
        # existing BlasterFile path so types remain correct and nothing is added.
        with open(blaster_files.low_dielectric_spheres_pdb_file.path, "w") as _f:
            _f.write("END\n")  # minimal valid PDB
