...
c        integer, dimension(:), allocatable :: conf_sorted ! gist need the confs to be in a particular order.
...
c      if (allocated(db2lig%conf_sorted)) then
c        if (size(db2lig%conf_sorted) .lt. total_confs) then
c          deallocate(db2lig%conf_sorted)
c        endif
c      endif
c      if (.not. allocated(db2lig%conf_sorted)) then
c        allocate(db2lig%conf_sorted(total_confs), stat=alloc_stat)
c        if (alloc_stat .ne. 0) then
c          write(OUTDOCK, *)
c     &        "ERROR ---> Failed to  allocate conf_sorted"
c          stop
c        endif
c      endif
