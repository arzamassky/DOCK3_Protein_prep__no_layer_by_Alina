...
c Keeping below comment for posterity
c           ! WHAT I WANT TO DO.  find out which confs (rigid segement) are connected. 
c           ! I can do this by looking at the bond information. 
c           ! look at which bonds spans two confs. id which segments
c           ! contain that atoms in the bonds. 
c           ! loop over all of the sets of confs and store which confs
c           ! are conected to witch
c           ! we have to loop over each set because one set is a full
c           ! molcule, there is overlap in atoms numbers for the sets.
c           ! for example sets 5 and 6 might have the same atoms. but 5 is
c           ! connected to 2 while 6 is connected to 3.
c  Ben Tingle : Update 8/4/2022 : this code has been troublesome
c  Going to scrap it, as it is only related to gist grid scoring
c  Which is not used much at the current moment
c  Want to bring back, but algorithm below needs a rehaul
c  Keeping this snippet of code as a reminder of what variables used to
c  be set in this block of code. check github if you're really curious
c            
c           do indtc = 1, db2lig%total_confs ! loop over all rigid segements (confs)
c              !write(*,*) indtc
c              db2lig%conf_conection(indtc) = indtc ! set conf_conection equal to itself
c              db2lig%conf_sorted(indtc) = indtc ! set to itself, if options0%use_gist is false
c           enddo
